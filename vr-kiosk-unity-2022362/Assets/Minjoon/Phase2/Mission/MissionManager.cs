...
namespace Phase2.Mission
{
    ...
    public class MissionManager : MonoBehaviour
    {
        ...
        // BUG-06: 씬 전환 시 싱글톤 및 정적 이벤트 정리
        private void OnDestroy()
        {
            if (Instance == this)
            {
                Instance = null;
                OnMissionTextUpdated = null;
                OnMissionValidated = null;
            }
        }
        ...
        public bool ValidateMission(IReadOnlyList<Core.CartItem> finalOrder)
        {
            ...
            foreach (var mission in _activeMissions)
            {
                ...
                {
                    Debug.Log($"[MissionManager] 미션 실패: {mission.MenuItem.menuName} 수량 불일치 (기대:{mission.Quantity}, 실제:{matchedCart.Quantity})");
                    OnMissionValidated?.Invoke(false);
                    return false;
                }
            }
        ...
        private bool OptionsMatch(List<CafeMenuOption> a, List<CafeMenuOption> b)
        {
            ...
            // 다중 옵션 중복 시 (예: 얼음, 얼음) 해시셋 오판을 방지하기 위한 Frequency Tally
            var aTally = new Dictionary<string, int>();
            foreach (var opt in a) 
            {
                if (aTally.ContainsKey(opt.optionId)) aTally[opt.optionId]++;
                else aTally[opt.optionId] = 1;
            }
            
            foreach (var opt in b)
            {
                if (!aTally.ContainsKey(opt.optionId) || aTally[opt.optionId] <= 0) return false;
                aTally[opt.optionId]--;
            }
            return true;
        }
        ...
        // BUG-01: HasJongseong → KoreanTextUtil로 이전 (dead code 제거)
    }
}
